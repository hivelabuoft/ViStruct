...
export default function Home({ totalQuestions }: { totalQuestions: number }) {
  ...
  // On component mount, check localStorage for a previously stored API key.
  useEffect(() => {
    const storedApiKey = localStorage.getItem("geminiApiKey");
    if (storedApiKey) {
      setApiKey(storedApiKey);
      // Optionally, you might want to auto-validate the key here
      // validateApiKey();
    }
  }, []);
  ...
  const validateApiKey = async () => {
    ...
    try {
      ...
      if (data.isValid) {
        // Store the API key in localStorage for future visits.
